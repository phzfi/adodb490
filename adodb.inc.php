...
*/
    ...
    static private $startedTrans = array();
    static private $endedTrans = array();
    
    static public function getStartedTrans()
    {
        return self::$startedTrans;
    }
    static public function getEndedTrans()
    {
        return self::$endedTrans;
    }
	...
	*/
	...
	{
//        self::$startedTrans[] = adodb_backtrace(false);
	...
	*/
	...
	{
//        self::$endedTrans[] = adodb_backtrace(false);
//        array_pop(self::$startedTrans);
	...
	*/
	...
	{
		//var_dump($this->check_query);
		if ($this->check_query)
		{
			$this->query_counter++;
			$GLOBALS['query_counter']++;
			
			if ($this->query_counter != $GLOBALS['query_counter'])
			{
				if ($GLOBALS['debug'] > 3)
				{
					print($GLOBALS['prev_callback']);
				}
				trigger_error('BROKEN sql object', E_USER_WARNING);
			}
			if ($GLOBALS['debug'] > 3)
			{
				$GLOBALS['prev_callback'] = adodb_backtrace(false);
			}
		}
		if ($this->debug) {
...
*/
	...
	function UnixTimeStamp($v)
	{
		...
		// odbtp support
		//( [year] => 2004 [month] => 9 [day] => 4 [hour] => 12 [minute] => 44 [second] => 8 [fraction] => 0 )
			$ts = adodb_mktime($v->hour,$v->minute,$v->second,$v->month,$v->day, $v->year);
			return $ts;
		}
		...
		if (!isset($rr[5]))
		{
			$ts = adodb_mktime(0,0,0,$rr[2],$rr[3],$rr[1]);
			return $ts;
		}
		$ts = @adodb_mktime($rr[5],$rr[6],$rr[7],$rr[2],$rr[3],$rr[1]);
		return $ts;
	}
